...
class Evaluator(object):
    ...
    def run(self, model_path, model_indice, log_file, log_file_link):
        ...
        if '.pth' in model_indice:
            ...
            start_epoch = int(model_indice.split("_")[0])
            end_epoch = model_indice.split("_")[1]
            ...
            if os.path.exists(model_path):
                models = [os.path.join(model_path, 'checkpoint_%s.pth' % model_indice), ] #here I changed 'epoch-%s.pth' to 'epoch-%s.pth.tar'
            else:
                models = [None]
