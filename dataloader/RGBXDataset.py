...
class RGBXDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        if self._file_length is not None:
            item_name = self._construct_new_file_names(self._file_length)[index].strip()
        else:
            item_name = self._file_names[index].strip()
        ...
        # here is only for SUNRGBD dataset
        # parts = item_name.split(' ')
        # rgb_name, x_name, gt_name = parts[0], parts[1], parts[2]
        # rgb_path = os.path.join(self._rgb_path, rgb_name)
        # x_path = os.path.join(self._x_path, x_name)
        # gt_path = os.path.join(self._gt_path, gt_name)
        ...
        # CMX read the gt image
        # gt = self._open_image(gt_path, cv2.IMREAD_GRAYSCALE, dtype=np.uint8)
        # print('gt:', gt.shape)
        # print('gt class:', np.unique(gt))

        # for deliver dataset, comment the line 56
        gt = io.read_image(gt_path)[0,...].unsqueeze(0)
        gt[gt==255] = 0
        gt -= 1
        gt = gt.squeeze().numpy()
        # deliver specific gt processing ends here
        
        if self._transform_gt:
            gt = self._gt_transform(gt) 

        #add a module to check the channels of x
        if self._x_single_channel:
            x = self._open_image(x_path, cv2.IMREAD_GRAYSCALE)
            x = cv2.merge([x, x, x])
        else:
            bgr_event = cv2.imread(x_path)
            x = cv2.cvtColor(bgr_event, cv2.COLOR_BGR2RGB)
