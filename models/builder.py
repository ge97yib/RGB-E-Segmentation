...
class EncoderDecoder(nn.Module):
    ...
    def encode_decode(self, rgb, modal_x):
        ...
        #print('modal_x shape:', modal_x.shape)
        if rgb.shape != modal_x.shape:
            # 计算需要添加的padding
            # padding格式：(左, 右, 上, 下)
            padding = (0, 0, (rgb.shape[2] - modal_x.shape[2]) // 2,
                       rgb.shape[2] - modal_x.shape[2] - (rgb.shape[2] - modal_x.shape[2]) // 2)
            modal_x = F.pad(modal_x, padding, "constant", 0)  # 使用0填充
            #print('New modal_x shape after padding:', modal_x.shape)
        # 确保padding后的尺寸正确
        assert rgb.shape == modal_x.shape, "Shape mismatch after padding!"

        x, stage_loss = self.backbone(rgb, modal_x)
        out = self.decode_head.forward(x)
        out = F.interpolate(out, size=orisize[2:], mode='bilinear', align_corners=False)
        if self.aux_head:
            aux_fm = self.aux_head(x[self.aux_index])
            aux_fm = F.interpolate(aux_fm, size=orisize[2:], mode='bilinear', align_corners=False)
            return out, aux_fm, stage_loss
        return out, stage_loss

    def forward(self, rgb, modal_x, label=None):
        if self.aux_head:
            out, aux_fm, stage_loss = self.encode_decode(rgb, modal_x)
        else:
            out, stage_loss = self.encode_decode(rgb, modal_x)
            #print('out shape:', out.shape)
        if label is not None:
            # for ce loss
            loss = self.criterion(out, label.long())
            # for kl loss
            # ce_loss = self.criterion(out, label.long())
            # loss = self.alpha*ce_loss + self.beta*stage_loss
