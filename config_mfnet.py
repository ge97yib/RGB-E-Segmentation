...
C.root_dir = '/mnt/hdd/my/MFNet' #os.path.abspath(os.path.join(os.getcwd(), './'))

C.abs_dir = osp.realpath(".")

# Dataset config
"""Dataset Path"""
C.dataset_name = 'MFNet' #'evscape4rgbx'
C.dataset_path = osp.join(C.root_dir, C.dataset_name)
C.rgb_root_folder = osp.join(C.dataset_path, 'rgb')  #rgb
C.rgb_format = '.jpg'
C.gt_root_folder = osp.join(C.dataset_path, 'labels')  #label
C.gt_format = '.png'
C.gt_transform = False
# True when label 0 is invalid, you can also modify the function _transform_gt in dataloader.RGBXDataset
# True for most dataset valid, Faslse for MFNet(?)
C.x_root_folder = osp.join(C.dataset_path, 'ther') #event:polL_dolp_img
C.x_format = '.jpg'
...
C.downsample = True # True for Deliver, False for others
...
C.weight_decay = 0.01
C.batch_size = 2 #8
C.nepochs = 500 #60 for others, 40 for ddd17
