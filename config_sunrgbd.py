...
# Dataset config
"""Dataset Path"""
C.dataset_name = 'SUN_RGBD' #'evscape4rgbx'
C.dataset_path = osp.join(C.root_dir, C.dataset_name)
C.rgb_root_folder = C.dataset_path  #rgb
C.rgb_format = '.jpg'
C.gt_root_folder = C.dataset_path  #label
C.gt_format = '.png'
C.gt_transform = True
# True when label 0 is invalid, you can also modify the function _transform_gt in dataloader.RGBXDataset
# True for most dataset valid, Faslse for MFNet(?)
C.x_root_folder = C.dataset_path #event:polL_dolp_img
C.x_format = '.png'
C.x_is_single_channel = True # True for raw depth, thermal and aolp/dolp(not aolp/dolp tri) input
C.train_source = osp.join(C.dataset_path, "train37.txt")
C.eval_source = osp.join(C.dataset_path, "test37.txt")
C.is_test = False
...
"""Image Config"""
C.background = 255
C.image_height = int(480)
C.image_width = int(480)
...
C.weight_decay = 0.01
C.batch_size = 2 #8
C.nepochs = 250 #60 for others, 40 for ddd17
...
C.eval_flip = False
C.eval_crop_size = [530, 730] # [height, width]
