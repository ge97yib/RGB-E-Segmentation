...
C.root_dir = '/mnt/hdd/my/deliver' #os.path.abspath(os.path.join(os.getcwd(), './'))

C.abs_dir = osp.realpath(".")

# Dataset config
"""Dataset Path"""
C.dataset_name = 'DELIVER_depth' #'evscape4rgbx'
C.dataset_path = osp.join(C.root_dir, C.dataset_name)
C.rgb_root_folder = osp.join(C.dataset_path, 'img')  #rgb
C.rgb_format = '.png'
C.gt_root_folder = osp.join(C.dataset_path, 'semantic')  #label
C.gt_format = '.png'
C.gt_transform = False
# True when label 0 is invalid, you can also modify the function _transform_gt in dataloader.RGBXDataset
# True for most dataset valid, Faslse for MFNet(?)
C.x_root_folder = osp.join(C.dataset_path, 'depth') #event:polL_dolp_img
C.x_format = '.png'
...
"""Image Config"""
C.background = 255
C.image_height = int(0.5*1042)
C.image_width = int(0.5*1042)
# C.image_height = int(1042)
# C.image_width = int(1042)
C.resize = False #True # True for Deliver, MFNet, NYUdepth, False for others
C.resize_scale = 0.5
...
C.pretrained_model = C.model_path + '/mit_b2.pth'
...
C.weight_decay = 0.01
C.batch_size = 2 #8
C.nepochs = 60 #60 for others, 40 for ddd17
C.niters_per_epoch = C.num_train_imgs // C.batch_size + 1
C.num_workers =2 #16
C.train_scale_array = [0.5, 0.75, 1, 1.25, 1.5, 1.75]#[0.75, 1, 1.25] for other datasets except SUNRGBD, DELIVER, others:[0.5, 0.75, 1, 1.25, 1.5, 1.75] 
C.warm_up_epoch = 10
...
C.eval_flip = False
C.eval_crop_size = [int(0.5*1042), int(0.5*1042)] # [height, width]
#C.eval_crop_size = [int(1042), int(1042)]
...
C.log_dir = osp.abspath('log_' + C.dataset_name + '_' + C.backbone + '_linear_crop') #re-trian for event crop CAFR
