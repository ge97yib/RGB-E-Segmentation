...
C.root_dir = '/mnt/hdd/my/dsec' #os.path.abspath(os.path.join(os.getcwd(), './'))

C.abs_dir = osp.realpath(".")

# Dataset config
"""Dataset Path"""
C.dataset_name = 'all_dsec' #'evscape4rgbx'
C.dataset_path = osp.join(C.root_dir, C.dataset_name)
C.rgb_root_folder = osp.join(C.dataset_path, 'rgb_images')  #rgb
C.rgb_format = '.png'
C.gt_root_folder = osp.join(C.dataset_path, 'label_folder')  #label
C.gt_format = '.png'
C.gt_transform = False
# True when label 0 is invalid, you can also modify the function _transform_gt in dataloader.RGBXDataset
# True for most dataset valid, Faslse for MFNet(?)
C.x_root_folder = osp.join(C.dataset_path, 'events_images') #event:polL_dolp_img
C.x_format = '.png'
...
"""Image Config"""
C.background = 255
C.image_height = int(440)
C.image_width = int(640)
...
C.num_workers =2 #16
C.train_scale_array = [0.5, 0.75, 1, 1.25, 1.5, 1.75] # [0.75, 1, 1.25] for SUNRGBD
C.warm_up_epoch = 10
...
C.eval_flip = False
C.eval_crop_size = [440, 640] # [height, width]
...
C.log_dir = osp.abspath('log_' + C.dataset_name + '_' + C.backbone + '_CAFR_new')
