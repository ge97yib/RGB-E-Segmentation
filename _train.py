...
with Engine(custom_parser=parser) as engine:
    ...
    for epoch in range(engine.state.epoch, config.nepochs+1):
        ...
        for idx in pbar:
            ...
            #     f"Labels out of range: min={torch.min(gts)}, max={torch.max(gts)}, unique labels={unique_labels}"
            #
            # # Optional: Handle labels out of range
            # gts[gts >= config.num_classes] = 255  # 将超出类别范围的标签设为 ignore_index
            # # print the name of the out of range labels
            # if torch.max(gts) == 255:
            #     print(f"Labels out of range: {torch.unique(gts)}")
